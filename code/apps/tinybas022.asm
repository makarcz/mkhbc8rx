...
;-----------------------------------------------------------------------------
; File: tinybas022.asm
; Purpose: Tiny BASIC port for MKHBC-8-Rx hobby computer.
; Created: 2/13/2018
; Copyright: TB is Copyright of Tom Pittman.
;-----------------------------------------------------------------------------
;
...
;
;
; Revision History (original work):
;
; v0.2.2 - 2011/11/11
...
;
;-----------------------------------------------------------------------------
; The End of original work copyright and info.
;
; Begin of MKHBC-8-Rx port info:
;
; This version compiles with CA65.
;
; Memory map:
;
;   TB resides at: $4400 - ...
;   ML code space: $5000 - $5FFF ( 4 kB)
;   Basic program: $6000 - $BFFF (23 kB)
;
; Revision history:
;
; 2/13/2018
...
.include "mkhbcos_ml.inc"   ; load definitions for MKHBCOS API
...
SSS:      .byte $20                   ; Spare Stack size. (was $04 but
                                      ; documentation suggests $20)
...
SRVT:    .word  IL_BBR               ; ($40-$5F) Backward Branch Relative
         ...
         .word  ILRES1               ; ($28) (Seems to be reserved - No IL
                                     ; opcode calls this)
         .word  ILRES2               ; ($29) (Seems to be reserved - No IL
                                     ; opcode calls this)
...
ERRSTR:  .byte $20,$41,$54,$20       ; " AT " string used in error reporting.
...
MEM_T:   lda ($22),Y                ; Load accumulator With the contents of
                                    ; a byte of memory
         tax                        ; Save it to X
         eor #$FF                   ; Next 4 instuctions test to see if this
                                    ; memeory location
         sta ($22),Y                ; is ram by trying to write something new
                                    ; to it - new value
         cmp ($22),Y                ; gets created by XORing the old value
                                    ; with $FF - store the
         php                        ; result of the test on the stack to look
                                    ; at later
...
SKP_PI:  plp                        ; Now look at the result of the memory test
         beq MEM_T                  ; Go test the next mempry location if the
                                    ; last one was ram
         dey                        ; If last memory location did not test as
                                    ; ram, decrement Y (should be $00 now)
IL__MT:  cld                        ; Make sure we're not in decimal mode
         lda $20                    ; Load up the low-order by of the start of
                                    ; free ram
         adc SSS                    ; Add to the spare stack size
         sta $24                    ; Store the result in $0024
         tya                        ; Retrieve Y
         adc $21                    ; And add it to the high order byte of the
                                    ; start of free ram (this does not look
                                    ; right)
         sta $25                    ; Store the result in $0025
         tya                        ; Retrieve Y again
         sta ($20),Y                ; Store A in the first byte of program
                                    ; memory
         iny                        ; Increment Y
         sta ($20),Y                ; Store A in the second byte of program
                                    ; memory
;
;Begin Warm Start
;
...
;
LBL005:  cmp #$30                   ;
         bcs LBL011                 ; If it's $30 or higher, it's a Branch or
                                    ; Jump - go handle it
         cmp #$08                   ;
         bcc LBL007                 ; If it's less than $08 it's a stack
                                    ; exchange - go handle it
         asl                        ; Multiply the OP code by 2
         tax                        ; Transfer it to X
LBL022:  lda SRVT-$03,X             ; Get the hi byte of the OP Code handling
                                    ; routine
         pha                        ; and save it on the stack
         lda SRVT-$04,X             ; Get the lo byte
         pha                        ; and save it on the stack
         php                        ; save the processor status too
         rti                        ; now go execute the OP Code handling
                                    ; routine
;
;
...
;
LBL043:  jsr LBL042
IL__PC:  jsr LBL004        ; Entry point for TBIL PC (print literal) - Go read
                           ; a byte from the TBIL table
         bpl LBL043
LBL042:  inc $BF
         bmi LBL044
...
IL__LB:  jsr LBL004                 ; Entry point for TBIL LB - Go read a byte
         jmp LBL029                 ; from the IL table
LBL085:  stx $2D
         cpx #$00
         rts
;
...
ILRES1:  sty $BC                    ;  does not seem to get called.
         ldy #$29                   ;  Need more research.
         sty $BD
         ldy #$00
         lda ($BC),Y
         cmp #$08
...
;
ILTBL:
.byte $24, $3E, $91, $27, $10, $E1, $59, $C5, $2A, $56, $10, $11, $2C, $8B, $4C
...
;
; Print a message.
; This sub expects the message offset from MBLK in Y.
;
SNDMSG:  lda MBLK,y                 ; Get a character from the message block
...
RCCHR:   jsr GetCh                  ; wait for character and get it in Acc
...
SNDCHR:  sta $FE                    ; Save the character to be printed
         ...
         cmp #$80	                  ;
         BEQ EXSC	                  ;
         jsr PutCh                  ; MKHBC-8-Rx, send character
         lda $fe                    ; restore the saved char to Acc
         rts
